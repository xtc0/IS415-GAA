...
jakarta_vac_points <- jakarta_vac_points %>% 
  dplyr::rename(
    ...
    #Vaccinated_All=JUMLAH..DOSIS.1, 
    #Vaccinated_Elderly=LANSIA..DOSIS.1,
    #Vaccinated_Public_Servants=PELAYAN.PUBLIK..DOSIS.1,
    #Vaccinated_Natural_Coop=GOTONG.ROYONG..DOSIS.1,
    #Vaccinated_Health_Workers=TENAGA.KESEHATAN..DOSIS.1,
    #Vaccinated_Stage3=TAHAPAN.3..DOSIS.1,
    #Vaccinated_Teenagers=REMAJA..DOSIS.1
    )
```
...
Why are these places having the highest vaccination rates? Who are the major contributors?

Let's first look for the City regions for the places with the greatest vaccination rate for a macroscopic view.

```{r}
#combined_jakarta$City[which.max(combined_jakarta$`Vaccinated`)]

#create a new column maybe for Vaccinated
```

1) Surprisingly, the greatest number of vaccinated people come from West Jakarta (the turquoise region from the map). However, from the choropleth maps, we can tell that the vaccination rates are low. Why? West Jakarta has the greatest number of people - out of which, a very large proportion of them are not vaccinated which causes the vaccination rate of West Jakarta to be low.

2) Vaccination rate of North Jakarta (blue) used to be high from July 2021 to December 2021. After which, the vaccination rate decreased. Why? What event caused this? Who?
Are we able to view the sub_districts from the map and see who's living there?
